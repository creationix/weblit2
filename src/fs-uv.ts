import { Flags, Fs, fs, StatEntry } from "uv";
/**
...
export function read(fd: number, data: Uint8Array, position: number, onRead: OnResult<number> = noop): void {
    const safeData = workaroundPrep(data);
...
export function fstatSync(fd: number): StatEntry {
    return fs.fstat(new Fs(), fd);
}
export function fstat(fd: number, onStat: OnResult<StatEntry> = noop): void {
    fs.fstat(new Fs(), fd, onStat);
}
